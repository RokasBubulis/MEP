...
function adjoint_action_by_campbell!(res, X::SparseMatrixCSC{TX, Int}, 
    ...
    # normY = norm(Y)
    # res ./= normY
    return nothing # normalisation enables machine precision for the trivial drift only case
end
...
function adjoint_drift!(tmp::Matrix{TCostate}, neg_im_drift::SparseMatrixCSC{TSystem,Int}, 
    im_control::SparseMatrixCSC{TSystem,Int}, α::TAlpha, stor::Storage) where {TAlpha, TSystem, TCostate}
...
function adjoint_drift(neg_im_drift::SparseMatrixCSC{TSystem,Int},
    im_control::SparseMatrixCSC{TSystem,Int}, α::TAlpha, stor::Storage) where {TAlpha, TSystem}
...
function neg_adjoint_drift_obj(x::Vector{TAlpha}, neg_im_drift::SparseMatrixCSC{TSystem,Int},
    im_control::SparseMatrixCSC{TSystem,Int}, costate::Matrix{TCostate}, stor::Storage) where {TAlpha, TSystem, TCostate}
    α = x[1]
    control_H = adjoint_drift(neg_im_drift, im_control, α, stor)
...
function neg_adjoint_drift_obj_1st_der!(G::AbstractVector{TAlpha}, x::AbstractVector{TAlpha}, 
    ...
    costate::Matrix{TCostate}, stor::Storage) where {TAlpha, TSystem, TCostate}
    α = x[1]
    control_H = adjoint_drift(neg_im_drift, im_control, α, stor)
...
function optimal_adjoint_drift_optimiser!(tmp::Matrix{TCostate}, costate::Matrix{TCostate}, system::System, stor::Storage) where TCostate
    neg_im_drift = -system.im_drift
    im_control = system.im_control
    x0 = [0.0]

    td = TwiceDifferentiable(
    x -> neg_adjoint_drift_obj(x, neg_im_drift, im_control, costate, stor),
    (G, x) -> neg_adjoint_drift_obj_1st_der!(G, x, neg_im_drift, im_control, costate, stor),
    (H, x) -> neg_adjoint_drift_obj_2nd_der!(H, x, neg_im_drift, im_control, costate, stor),
    x0
    )
    res = Optim.optimize(td, x0, Newton(linesearch = LineSearches.BackTracking()))
    α_optimal = Optim.minimizer(res)[1]

    adjoint_drift!(tmp, neg_im_drift, im_control, α_optimal, stor)
...
function optimal_adjoint_drift_analytic!(tmp::Matrix{TCostate}, costate::Matrix{TCostate}, system::System, solver::SolverParams, stor::Storage) where TCostate
    neg_im_drift = -system.im_drift
    im_control = system.im_control
    α = zero(real(TCostate))

    for _ in 1:solver.Newton_steps
        first_der = neg_adjoint_drift_obj_1st_der(α, neg_im_drift, im_control, costate, stor)
        second_der = neg_adjoint_drift_obj_2nd_der(α, neg_im_drift, im_control, costate, stor)
        dα = first_der / second_der
        α -= first_der / second_der
        abs(dα) < solver.Newton_tol && break
    end 

    adjoint_drift!(tmp, neg_im_drift, im_control, α, stor)
    # ensure optimal adjoint drift is anti-hermitian
    check_anti_hermiticity(tmp)
...
optimal_adjoint_drift!(tmp::Matrix{ComplexF64}, costate::Matrix{ComplexF64}, system::System, solver::SolverParams, stor::Storage
) = optimal_adjoint_drift_optimiser!(tmp, costate, system, stor)

optimal_adjoint_drift!(tmp::Matrix{<:Complex{<:ForwardDiff.Dual}}, 
costate::Matrix{<:Complex{<:ForwardDiff.Dual}}, system::System, solver::SolverParams, stor::Storage
) = optimal_adjoint_drift_analytic!(tmp, costate, system, solver, stor)