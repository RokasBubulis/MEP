...
title_str = latexstring("\\operatorname{Target:}\\;C_$(n_qubits-1)Phase(ϕ/π=$(phase/pi),\\; $gate_type), \\quad $(solver.tstar_min) <t_{optimal}<$(solver.tstar_max)")
println("Setup finished")

##
m_warm, dist, m_log = find_best_initial_costate_bbo(tar, algebra, solver, dt=dt, verbose=verbose, max_evals=50000, logging=true)
dt=1e-2
m_best = find_best_initial_costate(tar, algebra, solver, m0=m_warm, dt=dt, show_trace=verbose)
...
m0 /= norm(m0)
m_warm, d_best = find_best_initial_costate_cma(tar, algebra, solver, dt=dt, sigma=1.0, iterations=200)
m_best = find_best_initial_costate(tar, algebra, solver, m0=m_warm, dt=dt, show_trace=verbose)
min_dist, tstarbest, Ulistbest, αlistbest = propagate(m_best, algebra, solver, tar, dt=dt, save=true, show_trace=verbose)
println("dt: $dt, dmin=$min_dist, t*=$tstarbest")
##
...
p1 = plot(ts, dists_best[1:end], yscale=:log10, xlabel="Ωt", ylabel=L"d_{\min}")
title!(p1, title_str, title_font_size=12)
savefig(p1, file_name *"dist_vs_time_long.png")
display(p1)

# control plot
p2 = plot(ts[1:round(Int, tstarbest/dt)], αlistbest[1:round(Int, tstarbest/dt)]/(2pi),
xlabel="Ωt", ylabel=L"\operatorname{Optimal\;control}\;\; \frac{\bar{α}\,(t)}{2π}", grid=true, minorgrid=true, legend=false)
...
savefig(p2, file_name *"control_vs_time_long.png")
display(p2)
